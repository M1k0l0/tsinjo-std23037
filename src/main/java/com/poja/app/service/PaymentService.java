...
public class PaymentService {
  ...
  private final String volaBaseUrl =
      System.getProperty(
          "vola.base-url",
          "https://42cwka3n4ifcp7ufheyrpmph240iuaxo.lambda-url.eu-west-3.on.aws/v3/api-docs");
  ...
  public Payment createAndSubmit(Payment p) {
    ...
    try {
      ...
      ResponseEntity<Map> resp =
          volaRestTemplate.postForEntity(volaBaseUrl + "/payments", request, Map.class);
  ...
  private void startPolling(String externalId, Long paymentId) {
    ...
    Runnable task =
        () -> {
          ...
          try {
            // GET status
            ResponseEntity<Map> resp =
                volaRestTemplate.getForEntity(
                    volaBaseUrl + "/payments/{id}", Map.class, externalId);

            if (!resp.getStatusCode().is2xxSuccessful()) {
              log.warn(
                  "Polling non-2xx for externalId={}, status={}", externalId, resp.getStatusCode());
            ...
            String statusRaw =
                Optional.ofNullable(body.get("status")).map(Object::toString).orElse("UNKNOWN");
            String status = statusRaw.toUpperCase();
            log.debug(
                "Polling Vola: externalId={}, attempt={}, status={}", externalId, attempt, status);
            ...
            if ("SUCCEEDED".equals(status) || "FAILED".equals(status) || attempt >= maxAttempts) {
              ...
              if (payment != null) {
                ...
                log.info(
                    "Payment updated from polling: localId={}, externalId={}, newState={}",
                    payment.getId(),
                    payment.getReference(),
                    payment.getState());
              } else {
                log.warn(
                    "Payment not found localId={} when polling externalId={}",
                    paymentId,
                    externalId);
              }
            ...
            log.warn(
                "Erreur polling for externalId={} attempt={} : {}",
                externalId,
                attempts.get(),
                e.toString());
    ...
    ScheduledFuture<?> future =
        executor.scheduleAtFixedRate(task, initialDelaySec, pollingIntervalSec, TimeUnit.SECONDS);
