...
public class TsinjoController {
  ...
  public String index(Model model) {
    ...
    entries.addAll(
        donations.stream().map(this::toEntryDTOFromDonation).collect(Collectors.toList()));
  ...
  public String donate(
    ...
    // La validation minimale est faite par annotations HTML (required). Ici on fait un peu de
    // robustesse.
    if (email == null
        || email.isBlank()
        || fullName == null
  ...
  private EntryDTO toEntryDTOFromHelp(Help h) {
    String person =
        safeFullNameAndEmail(h.getBeneficiary().getFullName(), h.getBeneficiary().getEmail());
    return new EntryDTO(
        "HELP",
        person,
